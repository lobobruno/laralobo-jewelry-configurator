...
function findTypeUsage(typeName, allFiles) {
	...
	for (const file of allFiles) {
		...
		const patterns = [
			...
			new RegExp(
				`import\\s+type\\s*{[^}]*\\b${typeName}\\b[^}]*}\\s*from`,
				"g",
			),
			// import { type TypeName } from '...'
			new RegExp(
				`import\\s*{[^}]*\\btype\\s+${typeName}\\b[^}]*}\\s*from`,
				"g",
			),
		]
...
function getExistingTypeFiles() {
	...
	for (const dir of ALLOWED_DIRS) {
		const files = listFiles(dir, [".ts"]).filter(
...
function main() {
	...
	if (totalViolations.length > 0) {
		...
		totalViolations.forEach((violation, index) => {
			...
			console.log(
				`${RED}[${index + 1}]${RESET} ${violation.file}:${violation.line}`,
			)
			...
			if (violation.usedIn.length === 0) {
				console.log(
					`    ${GREEN}Fix: Remove 'export' keyword - type is not imported anywhere (local-only)${RESET}`,
				)
			} else {
				console.log(
					`    ${BLUE}Fix: Move to appropriate file in @/lib/types/ or @/lib/database/ and update imports in ${violation.usedIn.length} file(s)${RESET}`,
				)
		...
		if (existingTypeFiles.length > 0) {
			console.log(
				`${YELLOW}=== EXISTING TYPE FILES (for reference) ===${RESET}`,
			)
		...
		console.log(
			`Scanned: ${files.length} files | Violations: ${totalViolations.length} | Local-only: ${localOnly.length} | Shared: ${shared.length}`,
		)
		console.log()
		...
		totalViolations.forEach((violation) => {
			...
			console.log(
				`${violation.file}:${violation.line}:${violation.name}:${violation.kind}:${violation.usedIn.length}:[${fileList}]`,
			)
		})
		...
		console.log(
			`${GREEN}✓ No type export violations found (scanned ${files.length} files)${RESET}`,
		)
		process.exit(0)
	}
}
