...
export function estimateScreenScale(signals: ScreenSignals): { referenceWidth: number; label: string } | null {
    const { width, height, pixelRatio, viewportScale, userAgent } = signals;
    if (![width, height, pixelRatio, viewportScale].every(value => Number.isFinite(value) && value > 0)) return null;
    // A pinch-zoomed page must use manual calibration, not an uncorrected estimate.
    if (Math.abs(viewportScale - 1) > .001) return null;
    const physicalWidth = Math.min(width, height) * pixelRatio;
    const physicalHeight = Math.max(width, height) * pixelRatio;
    const matches = (profile: DisplayProfile) => Math.abs(profile.width - physicalWidth) <= 4 && Math.abs(profile.height - physicalHeight) <= 4;
    ...
    if (/iPhone/i.test(userAgent)) {
        ...
        const model = signals.model?.trim() || userAgent.match(/\b(Pixel 9|SM-S921[BUEWN0](?:1)?)(?=\s+Build|[;)/]|$)/i)?.[1] || '';
        if (/^Pixel 9$/i.test(model)) {
            ...
            profile = { width: 1080, height: 2340, ppi: Math.hypot(1080, 2340) / (156.4 / 25.4) };
            label = 'Samsung Galaxy S24';
        }
        if (profile && !matches(profile)) return null;
    }
    if (!profile) return null;
    const referenceWidth = Math.round((20 * profile.ppi / (25.4 * pixelRatio)) * 4) / 4;
...
export async function readDeviceModel(nav: NavigatorWithHints): Promise<string> {
    try {
