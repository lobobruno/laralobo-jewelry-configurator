...
export function createJewelry(state, gemGeometries) {
    ...
    function stoneSetting({ x = 0, y = 0, z = 0, size = 1, cut = state.cut, color = state.stone, prongs = true, parent = jewel, halo = false } = {}) {
        ...
        if (prongs) {
            for (let i = 0; i < 4; i++) {
                ...
                rod(V(px * .73, -sy * .48, pz * .73), V(px, sy * .23, pz), Math.min(.04, size * .045), mount, group);
    ...
    function buildRingHalo(geo, size, top, sy) {
        ...
        for (let row = 0; row < (state.halo === 'double' ? 2 : 1); row++) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Curved ribs join every diamond seat to the central basket.
                tubeCurve([V((x - normal.x * d * .5) * .84, top - sy * .38, (z - normal.z * d * .5) * .84), V(x * .94, y - h * .55, z * .94), V(x, y - h * .24, z)], d * .055, mount);
    ...
    function buildSignatureBasket(size, dims, top, sy) {
        ...
        const width = size * .79, baseY = top - sy * .52 - .055;
        ...
        const rx = size * .47, rz = size * dims.z * .43, frame = [];
        for (let i = 0; i < 64; i++) {
        ...
        for (const side of [-1, 1]) {
            rod(V(side * width * .48, baseY + .017, 0), V(side * rx, baseY, 0), .021, material, jewel);
            tubeCurve([V(side * 2.05 * Math.sin(.24), 2.05 * Math.cos(.24), 0), V(side * rx, baseY, 0)], .063, material);
            tubeCurve([V(side * rx, baseY, 0), V(side * size * .48, top - sy * .3, 0), V(side * size * .47, top - sy * .19, 0)], .026, material);
        }
        rod(V(0, baseY + .017, width * .46), V(0, baseY, rz), .019, material, jewel);
    }
    function buildRing() {
