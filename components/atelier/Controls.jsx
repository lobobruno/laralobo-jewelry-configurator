...
const CARAT_TICKS = [carats[0], carats[Math.floor(carats.length / 2)], carats.at(-1)].map(formatCt);
...
export default function Controls({ state, choose }) {
    return <div id="config-controls">
        {visibleSections(state, choose).map((section, index) => cloneElement(section, { step: index + 1 }))}
    </div>;
}
...
function visibleSections(state, choose) {
    ...
    return [
        ...
        ...(isPlainBand ? [
            ...
            state.family === 'ring' && <DetailsSection key="details" {...props} />
        ])
    ].filter(Boolean);
}
...
function ModelSection({ step, state, choose }) {
    return <Section step={step} label="O modelo">
        <OptionRow field="model" options={families[state.family].models} state={state} choose={choose} />
    </Section>;
}

function MetalSection({ step, state, choose }) {
    return <Section step={step} label="O metal" extra="18k">
        <div className="metal-options">
            {Object.entries(metals).map(([id, metal]) =>
                <Choice key={id} variant="metal-option" field="metal" value={id} state={state} choose={choose}>
                    <i className="swatch" style={{ background: metal.css }} />{metal.label}
                </Choice>
            )}
        </div>
    </Section>;
...
function CutSection({ step, state, choose }) {
    return <Section step={step} label="A lapidação">
        <div className="options">
            {Object.entries(cuts).map(([id, label]) =>
                <Choice key={id} variant="shape-option" field="cut" value={id} state={state} choose={choose}>
                    <Icon name={id} />{label}
                </Choice>
            )}
        </div>
...
function StoneSection({ step, state, choose }) {
    return <Section step={step} label="A pedra">
        <div className="stone-selector">
            {Object.entries(stones).map(([id, stone]) =>
                <Choice key={id} variant="stone-dot" field="stone" value={id} name={stone.name} state={state} choose={choose}>
...
function CaratSection({ step, state, choose }) {
    ...
    return <Section step={step} label={state.family === 'ring' ? 'O tamanho da pedra' : 'O tamanho das pedras'}>
...
function DetailsSection({ step, state, choose }) {
    return <Section step={step} label="Os detalhes">
        <OptionRow field="halo" options={HALOS} state={state} choose={choose} />
        <OptionRow field="bandStyle" options={BAND_STYLES} state={state} choose={choose} style={{ marginTop: 8 }} />
    </Section>;
}

function WidthSection({ step, state, choose }) {
    return <Section step={step} label="A largura">
        <Slider
...
function FinishSection({ step, state, choose }) {
    return <Section step={step} label="O acabamento">
        <OptionRow field="finish" options={FINISHES} state={state} choose={choose} />
    </Section>;
}
...
function Section({ step, label, extra, children }) {
    return <section className="control-section">
        <div className="section-label">
            <span><span className="step">{String(step).padStart(2, '0')}</span>{label}</span>
...
function OptionRow({ field, options, state, choose, style }) {
    return <div className="options" style={style}>
        {Object.entries(options).map(([value, label]) =>
            <Choice key={value} field={field} value={value} state={state} choose={choose}>{label}</Choice>
        )}
    </div>;
}
 ...
 */
function Choice({ variant = 'option', field, value, state, choose, name, children }) {
...
function Slider({ readout, note, value, min, max, ticks, ariaLabel, ariaValueText, onChange }) {
    return <>
        <div className="size-line"><strong>{readout}</strong>{note && <small>{note}</small>}</div>
        <input
            type="range"
            min={min}
            max={max}
        ...
        />
        <div className="range-labels">{ticks.map(tick => <span key={tick}>{tick}</span>)}</div>
    </>;
}
